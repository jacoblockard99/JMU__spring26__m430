...
We have, by our discussion in the previous exercise:
\begin{gather*}
    e_H = \phi(g)^n = \phi(g)^{qm+r} = \phi(g)^{qm} \phi(g)^r = (\phi(g)^m)^q \phi(g)^r = \phi(g)^r \,.
\end{gather*}
Since $m$ is the smallest positive integer such that $\phi(g)^m = e_H$,
and since $r < m$, we conclude $r = 0$.
